...
rtl8201_Object_t RTL8201;
rtl8201_IOCtx_t  RTL8201_IOCtx = {ETH_PHY_IO_Init,
                                  ETH_PHY_IO_DeInit,
                                  ETH_PHY_IO_WriteReg,
                                  ETH_PHY_IO_ReadReg,
                                  ETH_PHY_IO_GetTick};
...
/* USER CODE END PHY_PRE_CONFIG */
  ...
  RTL8201_RegisterBusIO(&RTL8201, &RTL8201_IOCtx);

  /* Initialize the RTL8201 ETH PHY */
  RTL8201_Init(&RTL8201);

  if (hal_eth_init_status == HAL_OK)
  {
    PHYLinkState = RTL8201_GetLinkState(&RTL8201);

    /* Get link state */
    if(PHYLinkState <= RTL8201_STATUS_LINK_DOWN)
    {
    ...
    else
    {
      switch (PHYLinkState)
      {
      case RTL8201_STATUS_100MBITS_FULLDUPLEX:
        duplex = ETH_FULLDUPLEX_MODE;
        speed = ETH_SPEED_100M;
        break;
      case RTL8201_STATUS_100MBITS_HALFDUPLEX:
        duplex = ETH_HALFDUPLEX_MODE;
        speed = ETH_SPEED_100M;
        break;
      case RTL8201_STATUS_10MBITS_FULLDUPLEX:
        duplex = ETH_FULLDUPLEX_MODE;
        speed = ETH_SPEED_10M;
        break;
      case RTL8201_STATUS_10MBITS_HALFDUPLEX:
        duplex = ETH_HALFDUPLEX_MODE;
        speed = ETH_SPEED_10M;
        break;
      default:
...
/* USER CODE END ETH link init */
  ...
  if(netif_is_link_up(netif) && (PHYLinkState <= RTL8201_STATUS_LINK_DOWN))
  {
    HAL_ETH_Stop_IT(&heth);
    netif_set_down(netif);
    netif_set_link_down(netif);
  }
  else if(!netif_is_link_up(netif) && (PHYLinkState > RTL8201_STATUS_LINK_DOWN))
  {
    switch (PHYLinkState)
    {
    case RTL8201_STATUS_100MBITS_FULLDUPLEX:
