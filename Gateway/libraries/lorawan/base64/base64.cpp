...
uint8_t base64_chr[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
uint16_t base64_encode(const uint8_t *in, uint16_t in_len, uint8_t* out) {
	...
	for (i=0;i<in_len;i++) {
		s[j++]=*(uint8_t *)(in+i);
		if (j==3) {
			out[k+0] = base64_chr[ (s[0]&255)>>2 ];
			out[k+1] = base64_chr[ ((s[0]&0x03)<<4)+((s[1]&0xF0)>>4) ];
			out[k+2] = base64_chr[ ((s[1]&0x0F)<<2)+((s[2]&0xC0)>>6) ];
			out[k+3] = base64_chr[ s[2]&0x3F ];
			j=0;
			k+=4;
		}
	}
