...
HAL_StatusTypeDef HAL_ETH_ReadData(ETH_HandleTypeDef *heth, void **pAppBuff) {
	...
	while ((READ_BIT(dmarxdesc->DESC0, ETH_DMARXDESC_OWN) == (uint32_t) RESET)
		...
		if ((READ_BIT(dmarxdesc->DESC0, ETH_DMARXDESC_FS) != (uint32_t) RESET)
			...
			if (READ_BIT(dmarxdesc->DESC0, ETH_DMARXDESC_FS)
					!= (uint32_t) RESET) {
			...
			if (READ_BIT(dmarxdesc->DESC0, ETH_DMARXDESC_LS)
					!= (uint32_t) RESET) {
      ...
      heth->rxLinkCallback(&heth->RxDescList.pRxStart, &heth->RxDescList.pRxEnd,
			...
			HAL_ETH_RxLinkCallback(&heth->RxDescList.pRxStart,
					&heth->RxDescList.pRxEnd, (uint8_t*) dmarxdesc->BackupAddr0,
					(uint16_t) bufflength);
#endif  /* USE_HAL_ETH_REGISTER_CALLBACKS */
      ...
      heth->rxAllocateCallback(&buff);
		...
		if (allocStatus != 0U) {
			if (heth->RxDescList.ItMode == 0U) {
				...
				WRITE_REG(dmarxdesc->DESC1,
						ETH_RX_BUF_SIZE | ETH_DMARXDESC_RCH);
			}
...
HAL_StatusTypeDef HAL_ETH_GetRxDataErrorCode(ETH_HandleTypeDef *heth,
		uint32_t *pErrorCode) {
	/* Get error bits. */
	*pErrorCode = READ_BIT(heth->RxDescList.pRxLastRxDesc,
	...
	if (__HAL_ETH_DMA_GET_IT(heth, ETH_DMASR_AIS)) {
		if (__HAL_ETH_DMA_GET_IT_SOURCE(heth, ETH_DMAIER_AISE)) {
			...
			if (__HAL_ETH_DMA_GET_IT(heth, ETH_DMASR_FBES)) {
				...
				__HAL_ETH_DMA_DISABLE_IT(heth,
						ETH_DMAIER_NISE | ETH_DMAIER_AISE);
...
HAL_StatusTypeDef HAL_ETH_GetMACConfig(ETH_HandleTypeDef *heth,
	...
	macconf->RetryTransmission =
			((READ_BIT(heth->Instance->MACCR, ETH_MACCR_RD) >> 9) == 0U) ?
					ENABLE : DISABLE;
	macconf->CarrierSenseDuringTransmit =
			((READ_BIT(heth->Instance->MACCR, ETH_MACCR_CSD) >> 16) > 0U) ?
					ENABLE : DISABLE;
	macconf->ReceiveOwn =
			((READ_BIT(heth->Instance->MACCR, ETH_MACCR_ROD) >> 13) == 0U) ?
					ENABLE : DISABLE;
	...
	macconf->Jabber =
			((READ_BIT(heth->Instance->MACCR, ETH_MACCR_JD) >> 22) == 0U) ?
					ENABLE : DISABLE;
	macconf->Watchdog =
			((READ_BIT(heth->Instance->MACCR, ETH_MACCR_WD) >> 23) == 0U) ?
					ENABLE : DISABLE;
	macconf->AutomaticPadCRCStrip =
			((READ_BIT(heth->Instance->MACCR, ETH_MACCR_APCS) >> 7) > 0U) ?
					ENABLE : DISABLE;
	macconf->InterPacketGapVal = READ_BIT(heth->Instance->MACCR, ETH_MACCR_IFG);
	macconf->ChecksumOffload =
			((READ_BIT(heth->Instance->MACCR, ETH_MACCR_IPCO) >> 10U) > 0U) ?
					ENABLE : DISABLE;

	macconf->TransmitFlowControl =
			((READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_TFCE) >> 1) > 0U) ?
					ENABLE : DISABLE;
	macconf->ZeroQuantaPause =
			((READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_ZQPD) >> 7) == 0U) ?
					ENABLE : DISABLE;
	macconf->PauseLowThreshold = READ_BIT(heth->Instance->MACFCR,
			ETH_MACFCR_PLT);
	macconf->PauseTime =
			(READ_BIT(heth->Instance->MACFCR, ETH_MACFCR_PT) >> 16);
...
HAL_StatusTypeDef HAL_ETH_GetDMAConfig(ETH_HandleTypeDef *heth,
	...
	dmaconf->AddressAlignedBeats =
			((READ_BIT(heth->Instance->DMABMR, ETH_DMABMR_AAB) >> 25U) > 0U) ?
					ENABLE : DISABLE;
	dmaconf->BurstMode = READ_BIT(heth->Instance->DMABMR,
			ETH_DMABMR_FB | ETH_DMABMR_MB);
	dmaconf->RxDMABurstLength = READ_BIT(heth->Instance->DMABMR,
			ETH_DMABMR_RDP);
	dmaconf->TxDMABurstLength = READ_BIT(heth->Instance->DMABMR,
			ETH_DMABMR_PBL);
	dmaconf->EnhancedDescriptorFormat =
			((READ_BIT(heth->Instance->DMABMR, ETH_DMABMR_EDE) >> 7) > 0U) ?
					ENABLE : DISABLE;
	...
	dmaconf->ReceiveStoreForward =
			((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_RSF) >> 25) > 0U) ?
					ENABLE : DISABLE;
	dmaconf->FlushRxPacket =
			((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_FTF) >> 20) > 0U) ?
					DISABLE : ENABLE;
	dmaconf->TransmitStoreForward =
			((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_TSF) >> 21) > 0U) ?
					ENABLE : DISABLE;
	dmaconf->TransmitThresholdControl = READ_BIT(heth->Instance->DMAOMR,
			ETH_DMAOMR_TTC);
	dmaconf->ForwardErrorFrames =
			((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_FEF) >> 7) > 0U) ?
					ENABLE : DISABLE;
	dmaconf->ForwardUndersizedGoodFrames = ((READ_BIT(heth->Instance->DMAOMR,
			ETH_DMAOMR_FUGF) >> 6) > 0U) ? ENABLE : DISABLE;
	dmaconf->ReceiveThresholdControl = READ_BIT(heth->Instance->DMAOMR,
			ETH_DMAOMR_RTC);
	dmaconf->SecondFrameOperate =
			((READ_BIT(heth->Instance->DMAOMR, ETH_DMAOMR_OSF) >> 2) > 0U) ?
					ENABLE : DISABLE;
	return HAL_OK;
}
...
HAL_StatusTypeDef HAL_ETH_GetMACFilterConfig(ETH_HandleTypeDef *heth,
	...
	pFilterConfig->PromiscuousMode =
			((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_PM)) > 0U) ?
					ENABLE : DISABLE;
	pFilterConfig->HashUnicast =
			((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_HU) >> 1) > 0U) ?
					ENABLE : DISABLE;
	pFilterConfig->HashMulticast =
			((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_HM) >> 2) > 0U) ?
					ENABLE : DISABLE;
	...
	pFilterConfig->PassAllMulticast =
			((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_PAM) >> 4) > 0U) ?
					ENABLE : DISABLE;
	pFilterConfig->BroadcastFilter =
			((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_BFD) >> 5) == 0U) ?
					ENABLE : DISABLE;
	pFilterConfig->ControlPacketsFilter = READ_BIT(heth->Instance->MACFFR,
			ETH_MACFFR_PCF);
	...
	pFilterConfig->SrcAddrFiltering =
			((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_SAF) >> 9) > 0U) ?
					ENABLE : DISABLE;
	pFilterConfig->HachOrPerfectFilter =
			((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_HPF) >> 10) > 0U) ?
					ENABLE : DISABLE;
	pFilterConfig->ReceiveAllMode =
			((READ_BIT(heth->Instance->MACFFR, ETH_MACFFR_RA) >> 31) > 0U) ?
					ENABLE : DISABLE;
...
void HAL_ETH_EnterPowerDownMode(ETH_HandleTypeDef *heth,
	...
	powerdownconfig = (((uint32_t) pPowerDownConfig->MagicPacket
			<< ETH_MACPMTCSR_MPE_Pos)
			| ((uint32_t) pPowerDownConfig->WakeUpPacket
					<< ETH_MACPMTCSR_WFE_Pos)
			| ((uint32_t) pPowerDownConfig->GlobalUnicast
...
static void ETH_SetMACConfig(ETH_HandleTypeDef *heth,
	...
	tmpreg1 |=
			(uint32_t) (((uint32_t) ((macconf->Watchdog == DISABLE) ? 1U : 0U)
					<< 23U)
					| ((uint32_t) ((macconf->Jabber == DISABLE) ? 1U : 0U)
							<< 22U) | (uint32_t) macconf->InterPacketGapVal
					| ((uint32_t) macconf->CarrierSenseDuringTransmit << 16U)
					| macconf->Speed
					| ((uint32_t) ((macconf->ReceiveOwn == DISABLE) ? 1U : 0U)
							<< 13U) | ((uint32_t) macconf->LoopbackMode << 12U)
					| macconf->DuplexMode
	...
	tmpreg1 |= (uint32_t) ((macconf->PauseTime << 16U)
			| ((uint32_t) ((macconf->ZeroQuantaPause == DISABLE) ? 1U : 0U)
					<< 7U) | macconf->PauseLowThreshold
			| ((uint32_t) (
					(macconf->UnicastPausePacketDetect == ENABLE) ? 1U : 0U)
					<< 3U)
			| ((uint32_t) ((macconf->ReceiveFlowControl == ENABLE) ? 1U : 0U)
					<< 2U)
			| ((uint32_t) ((macconf->TransmitFlowControl == ENABLE) ? 1U : 0U)
					<< 1U));
...
static void ETH_SetDMAConfig(ETH_HandleTypeDef *heth,
	...
	tmpreg1 |=
			(uint32_t) (((uint32_t) (
					(dmaconf->DropTCPIPChecksumErrorFrame == DISABLE) ? 1U : 0U)
					<< 26U) | ((uint32_t) dmaconf->ReceiveStoreForward << 25U)
					| ((uint32_t) ((dmaconf->FlushRxPacket == DISABLE) ? 1U : 0U)
							<< 20U)
	...
	(heth->Instance)->DMABMR =
			(uint32_t) (((uint32_t) dmaconf->AddressAlignedBeats << 25U)
					| dmaconf->BurstMode | dmaconf->RxDMABurstLength | /* !! if 4xPBL is selected for Tx or
...
static void ETH_MACDMAConfig(ETH_HandleTypeDef *heth) {
	...
	dmaDefaultConf.ReceiveThresholdControl =
...
static void ETH_DMATxDescListInit(ETH_HandleTypeDef *heth) {
	...
	for (i = 0; i < (uint32_t) ETH_TX_DESC_CNT; i++) {
		...
		if (i < ((uint32_t) ETH_TX_DESC_CNT - 1U)) {
			WRITE_REG(dmatxdesc->DESC3,
					(uint32_t )(heth->Init.TxDesc + i + 1U));
		} else {
	...
	for (i = 0; i < (uint32_t) ETH_RX_DESC_CNT; i++) {
		...
		if (i < ((uint32_t) ETH_RX_DESC_CNT - 1U)) {
			WRITE_REG(dmarxdesc->DESC3,
					(uint32_t )(heth->Init.RxDesc + i + 1U));
		} else {
...
static uint32_t ETH_Prepare_Tx_Descriptors(ETH_HandleTypeDef *heth,
	...
	if (READ_BIT(pTxConfig->Attributes, ETH_TX_PACKETS_FEATURES_CSUM) != 0U) {
		MODIFY_REG(dmatxdesc->DESC0, ETH_DMATXDESC_CIC,
				pTxConfig->ChecksumCtrl);
	}

	if (READ_BIT(pTxConfig->Attributes, ETH_TX_PACKETS_FEATURES_CRCPAD) != 0U) {
		MODIFY_REG(dmatxdesc->DESC0, ETH_CRC_PAD_DISABLE,
